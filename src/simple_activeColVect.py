...
def calculateActiveColumnVect(activeCols, inhibCols, colOverlapVect):
    ...
    with tf.name_scope('get_activeColVect'):
        ...
        test_lcol = tf.cast(tf.greater_equal(w_cols, nonPaddingSumVect), tf.int32, name='test_meInhib')
        test_meInhib = tf.where(tf.greater(nonPaddingSumVect, 0), test_lcol, zeros_vec, name='test_meInhib')
    ...
    with tf.name_scope('print'):
        ...
        activeColumnVect = tf.Print(activeColumnVect,
                                   [activeColumnVect, colwinners, w_cols, test_lcol],
